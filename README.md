...
| `ai-mem-search <term> [project]` | Grep across the vault (or one project's logs) for a term, with `file:line:excerpt`. Also resolves any `[[wikilink]]` on a matched line to its project note and shows a short excerpt -- one hop out along the graph, always on, not a flag to remember |
